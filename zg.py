...
def get_filename_with_time_order(rootdir,ensemble,dates):
    '''get filename first and generate label ,one different w'''
    _files = []
    for en in ensemble:
        for date in dates:
            access_path=rootdir+en+"/"+"da_zg_"+date.strftime("%Y%m%d")+"_"+en+".nc"
#             print(access_path)
...
def main():
    ...
    ensemble=['e04','e05','e06','e07','e08','e09','e10','e11']

    var_name="zg"
    dates=[start_date + timedelta(x) for x in range((end_date - start_date).days + 1)]
    file_list=get_filename_with_time_order(file_ACCESS_dir+var_name+'/daily/',ensemble,dates)
    time_leading=7
...
#     print(file_list)
    for i in file_list:
        print(i)
        data = Dataset(i[0], 'r')
        var = data[var_name][:7,-3,82:144,134:188]
...
#         print(var.shape)
    ...
    #         lr=dpt.read_access_data(i,idx=time_leading).data[82:144,134:188]*86400
        ...
        if not os.path.exists('../data/zg/daily/'+i[1]):
            os.mkdir('../data/zg/daily/'+i[1])

        f_w = nc.Dataset('../data/zg/daily/'+i[1]+"/da_zg_"+i[2].strftime("%Y%m%d")+"_"+i[1]+'.nc','w',format = 'NETCDF4')
